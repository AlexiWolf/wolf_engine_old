...
/// ```ignore
/// # use legion_codegen::system;
/// # use wolf_engine::ecs::{Schedule, systems::CommandBuffer, world::SubWorld};
...
/// ```ignore
/// # use legion_codegen::system;
/// # use wolf_engine::ecs::{Schedule, world::SubWorld, Read, Write, IntoQuery};
/// # struct Time;
/// #[system]
...
/// ```ignore
/// # use legion_codegen::system;
/// # use wolf_engine::ecs::{Schedule, world::SubWorld, Read, Write, IntoQuery, Query};
/// # struct Time;
/// #[system]
...
/// ```ignore
/// # use legion_codegen::system;
/// # use wolf_engine::ecs::maybe_changed;
/// # struct Position { x: f32 }
/// # struct Velocity { x: f32 }
/// # struct Time { seconds: f32 }
/// #[system(for_each)]
...
/// ```ignore
/// # use legion_codegen::system;
/// # use wolf_engine::ecs::{storage::Component, Schedule};
/// # use std::fmt::Debug;
/// # #[derive(Debug)]
/// # struct Position;
/// #[system(for_each)]
                    ...
                    match arg.ty.as_ref() {
                                        ...
                                        GenericArgument::Type(ty) => {
                                            match ty {
                                                Type::Reference(ty) => {
                                                    ...
                                                    if mutable {
                                                        query.push(
                                                            parse_quote!(#prefix::wolf_engine::ecs::TryWrite<#elem>),
                                                        );
                                                    } else {
                                                        query.push(
                                                            parse_quote!(#prefix::wolf_engine::ecs::TryRead<#elem>),
                                                        );
                                                    }
                                                }
                                                _ => {
                        ...
                        Type::Reference(ty)
                            ...
                            || is_type(&ty.elem, &["legion", "world", "Entity"]) =>
                            {
                                ...
                                query.push(parse_quote!(#prefix::wolf_engine::ecs::Entity));
                            }
                        Type::Reference(ty)
                        ...
                        Type::Reference(ty) => {
                            ...
                            match attribute {
                                ...
                                None => {
                                    ...
                                    let elem = &ty.elem;
                                    if mutable {
                                        query.push(parse_quote!(#prefix::wolf_engine::ecs::Write<#elem>));
                                    } else {
                                        query.push(parse_quote!(#prefix::wolf_engine::ecs::Read<#elem>));
                                    }
                                }
                            }
                        }
...
impl Config {
    ...
    fn generate(&mut self) -> Result<proc_macro2::TokenStream, Error> {
        ...
        let builder = quote! {
            use wolf_engine::ecs::IntoQuery;
            #generic_parameter_names
            #prefix::wolf_engine::ecs::systems::SystemBuilder::new(format!("{}{}", #system_name, generic_names))
        ...
        let result = quote! {
            #visibility fn #constructor_name<#generic_params>(#(#fn_params),*) -> impl #prefix::wolf_engine::ecs::systems::Runnable
            #where_clause
            {
            #builder
            }
